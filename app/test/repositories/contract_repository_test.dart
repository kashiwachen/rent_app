...
void main() {
  ...
  group('ContractRepository - CRUD Operations', () {
    test(
      'createContract - should create contract with monthly cycle',
      () async {
        // Arrange
    ...
    test(
      'createContract - should throw when property does not exist',
      () async {
        // Arrange
        final tenantIds = await seedTestTenants(db);
    ...
    test(
      'getAllContracts - should return empty list when no contracts exist',
      () async {
        // Act
    ...
    test(
      'getActiveContracts - should return empty list when no active contracts',
      () async {
        // Arrange
        final propertyIds = await seedTestProperties(db);
        final tenantIds = await seedTestTenants(db);
        ...
        for (final id in contractIds) {
          await (db.update(db.contracts)..where((t) => t.id.equals(id))).write(
            const ContractsCompanion(isActive: Value(false)),
          );
        }
    ...
    test(
      'getContractById - should return null when id does not exist',
      () async {
        // Act
    ...
    test(
      'updateContract - should throw when contract does not exist',
      () async {
        // Arrange
    ...
    test(
      'terminateContract - should throw when contract does not exist',
      () async {
        // Act & Assert
  ...
  group('ContractRepository - Payment Schedule Generation', () {
    ...
    test(
      'generatePaymentSchedules - should generate 1 yearly schedule',
      () async {
        // Arrange
        final propertyIds = await seedTestProperties(db);
        final tenantIds = await seedTestTenants(db);
    ...
    test(
      'generatePaymentSchedules - should handle multi-year contracts',
      () async {
        // Arrange
        final propertyIds = await seedTestProperties(db);
        final tenantIds = await seedTestTenants(db);
    ...
    test(
      'generatePaymentSchedules - should handle February edge case',
      () async {
        // Arrange
        final propertyIds = await seedTestProperties(db);
        final tenantIds = await seedTestTenants(db);
    ...
    test(
      'generatePaymentSchedules - should throw when contract does not exist',
      () async {
        // Act & Assert
        expect(
  ...
  group('ContractRepository - Edge Cases', () {
    ...
    test(
      'createContract - should handle short-term contract (1 month)',
      () async {
        // Arrange
        final propertyIds = await seedTestProperties(db);
        final tenantIds = await seedTestTenants(db);
