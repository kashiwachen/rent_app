...
class CoreDataService: ObservableObject {
    ...
    func calculateYearlyIncome(year: Int) -> Decimal {
        ...
        let startDate = Calendar.current.date(from: DateComponents(year: year, month: 1, day: 1)) ?? Date()
        let endDate = Calendar.current.date(from: DateComponents(year: year + 1, month: 1, day: 1)) ?? Date()
        ...
        do {
            ...
            print("Failed to calculate yearly income: \(error)")
            return 0
        }
    }
    ...
    func calculateYearlyExpenses(year: Int) -> Decimal {
        ...
        do {
            ...
            print("Failed to calculate yearly expenses: \(error)")
            return 0
        }
    }
